...
class App extends Component {
  ...
  async getNFT2(val2) {
    const nft = await fetch(
      "https://api.opensea.io/api/v1/asset/0x740c178e10662bbb050bde257bfa318defe3cabc/" +
        val2
    );
  ...
  async getNFT(val, n) {
    ...
    if (nft.status !== 404 && nft.status !== 429 && nft.status !== 500) {
      ...
      if (n == 1) {
        ...
        if (data.last_sale.total_price) {
          this.state.nftpr1 = data.last_sale.total_price;
        }
      } else {
        this.setState({
        ...
        if (data.lastsale && data.last_sale.total_price) {
          this.state.nftpr2 = data.last_sale.total_price;
          console.log("hello", this.state.nftpr2);
        }
      }
    } else {
      this.state.atknft2 = "";
  ...
  attack() {
    if (this.state.atknft1 != "" && this.state.atknft2 != "") {
      const st =
        ...
        "NFT " +
        this.state.atknft2 +
        ". Please check the NFT you want to attack carefully";
  ...
  revivefunc() {
    ...
    if (answer) {
      ...
      } else alert("Choose acceptable NFT to revive");
    }
  }
